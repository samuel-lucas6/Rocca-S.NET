...
internal sealed class RoccaSx86 : IDisposable
{
    ...
    internal RoccaSx86(ReadOnlySpan<byte> key, ReadOnlySpan<byte> nonce)
    {
    ...
    {
        if (_disposed) { throw new ObjectDisposedException(nameof(RoccaSx86)); }
        int i = 0;
    ...
    private void Round(Vector128<byte> x0, Vector128<byte> x1)
    {
        ...
        Vector128<byte> sNew1 = Aes.Encrypt(_s[0], x0);
        Vector128<byte> sNew2 = Aes.Encrypt(_s[1], _s[0]);
        Vector128<byte> sNew3 = Aes.Encrypt(_s[2], _s[6]);
        Vector128<byte> sNew4 = Aes.Encrypt(_s[3], x1);
        Vector128<byte> sNew5 = Aes.Encrypt(_s[4], _s[3]);
        Vector128<byte> sNew6 = Aes.Encrypt(_s[5], _s[4]);
    ...
    private void Encryption(Span<byte> ciphertext, ReadOnlySpan<byte> plaintext)
    {
        ...
        Vector128<byte> c0 = Aes.Encrypt(_s[3] ^ _s[5], _s[0]) ^ m0;
        Vector128<byte> c1 = Aes.Encrypt(_s[4] ^ _s[6], _s[2]) ^ m1;
        Round(m0, m1);
    ...
    private void Decryption(Span<byte> plaintext, ReadOnlySpan<byte> ciphertext)
    {
        ...
        Vector128<byte> m0 = Aes.Encrypt(_s[3] ^ _s[5], _s[0]) ^ c0;
        Vector128<byte> m1 = Aes.Encrypt(_s[4] ^ _s[6], _s[2]) ^ c1;
        Round(m0, m1);
